...
| `Cannot write to cgroup hierarchy (/sys/fs/cgroup)` in container logs | Rootless Podman without cgroup v2 controller delegation from the host | One-time host setup: `sudo mkdir -p /etc/systemd/system/user@.service.d && sudo tee /etc/systemd/system/user@.service.d/delegate.conf <<'EOF'\n[Service]\nDelegate=cpu cpuset io memory pids\nEOF` then `sudo systemctl daemon-reload` and re-login |
| `open /dev/kmsg: operation not permitted` in container logs (rootless Podman) | User namespace restricts `/dev/kmsg` access; `KubeletInUserNamespace` feature gate not applied | Verify the entrypoint detects the user namespace (check for "Detected user namespace" in logs). If missing, the `/proc/self/uid_map` detection may have failed. The feature gate is set by `cluster-entrypoint.sh` when `uid_map` shows inner UID 0 mapped to non-zero outer UID |
| `failed to find cpuset cgroup (v2)` or `mkdir /sys/fs/cgroup/kubepods: permission denied` | Rootless Podman container cannot create cgroup hierarchy due to missing host delegation | Same fix as the `Cannot write to cgroup hierarchy` entry above. The pre-flight check in `docker.rs` (`check_rootless_cgroup_delegation`) should warn about this before deployment |
